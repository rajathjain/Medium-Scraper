...
import pandas as pd
import time
import Scrape_Story as ss
from tqdm import tqdm
import pickle

def url_generator(tag):
...
def article_link(cards):
    articles=[]
    for card in cards:
        article_link=card.find("a",class_="button button--smaller button--chromeless u-baseColor--buttonNormal")
        if article_link is not None: 
            
            articles.append(article_link["href"].split("?")[0])
        else:
            articles.append("NaN")
    return articles
...
def story(links):
    ...
    for link in tqdm(links):
        if link!="NaN":
            stories.append(ss.scrape_story(link))
        else:
            stories.append("NaN")
    return stories
...
#-----------------------------------------------------------------
    ...
    while(current_date <= end_date):
        ...
        articles=article_link(cards)
        stories=story(articles)
        
 
       #ACCUMULATE DATA INTO A DICTIONARY
        dict = {"Title":titles,"Subtitle":subtitles,"Image":images,"Author":authors, "Publication":pubs, "Year":year, "Month":month, "Day":day, "Tag":tags, "Reading_Time":readingTimes, "Claps":applause,"Comment":comment, "Author_url":auth_urls,"Article_Link":articles}
        ...
        #Pickling articles/stories
        if firstPage:
            story_dict={}
            for number in range(len(articles)):
                story_dict['article_link_'+str(number)] = articles[number]
                story_dict['article_content_'+str(number)] = stories[number]
                firstpage=False
                with open("Tag_Scrapes/stories.pickle","wb") as f:
            ...
            for number in range(len(articles)):
                ...
                story_dict['article_content_'+str(number)] = stories[number]
                firstpage=False
                with open("Tag_Scrapes/stories.pickle","ab") as f:
                    pickle.dump(story_dict,f)
